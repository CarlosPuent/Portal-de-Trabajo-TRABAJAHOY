...
export async function initAdminForumPage(params, query) {
  ...
  try {
    const [reportsData, threadsData, catsData, reviewReportsData] = await Promise.all([
    ...
    document.getElementById('app').innerHTML = getAdminForumHTML(reports, threads, categories, reviewReports, isAuthenticated, user);
    initAdminForumEvents(reports, threads, reviewReports);
  } catch (error) {
...
function getAdminForumHTML(reports, threads, categories, reviewReports, isAuthenticated, user) {
  ...
  const mainContent = `
    <div class="af-container">
      ...
      <div class="af-section-tabs">
        <button class="af-sec-tab af-sec-tab--active" data-section="reports">🚩 Foro (${pendingReports.length})</button>
        <button class="af-sec-tab" data-section="threads">📋 Hilos</button>
        <button class="af-sec-tab" data-section="categories">📁 Categorías</button>
        <button class="af-sec-tab" data-section="reviews">⭐ Reseñas (${pendingReviewReports})</button>
      </div>
            ...
            return `
              <div class="af-report" data-id="${report.id}" data-status="${report.status}" ${report.status !== 'pending' ? 'style="display:none;"' : ''}>
                <div class="af-report__header">
                  ...
                  <span class="af-report__type">${report.threadId ? '📝 Hilo' : '💬 Respuesta'}</span>
                ...
                ${report.status === 'pending' ? `
                  <div class="af-report__actions">
                    <button class="btn btn--primary btn--sm resolve-report-btn" data-id="${report.id}">✓ Resolver</button>
                    <button class="btn btn--outline btn--sm dismiss-report-btn" data-id="${report.id}">✗ Descartar</button>
                    ${report.threadId ? `<button class="btn btn--sm" style="background:#fee2e2;color:#dc2626;border:none;" data-thread-id="${report.threadId}" class="delete-reported-thread">🗑 Eliminar Hilo</button>` : ''}
                    ${report.postId ? `<button class="btn btn--sm delete-reported-post" style="background:#fee2e2;color:#dc2626;border:none;" data-post-id="${report.postId}">🗑 Eliminar Respuesta</button>` : ''}
                  </div>
                ` : ''}
              </div>
            `;
              ...
              <div class="af-thread-row" data-id="${t.id}">
                <div class="af-thread-row__info">
                  ...
                  <div class="af-thread-row__meta">
                    ...
                    ${t.isPinned ? '<span class="ar-tag" style="background:#fef3c7;color:#92400e;">📌 Fijado</span>' : ''}
                    ${t.isLocked ? '<span class="ar-tag" style="background:#fee2e2;color:#991b1b;">🔒 Cerrado</span>' : ''}
                  </div>
                </div>
            ...
            return `
              <div class="af-review-row" data-id="${r.id || review.id}">
                <div class="af-review-row__header">
                  ...
                  <span class="af-review-row__stars">★ ${review.rating || r.rating || '?'}/5</span>
                ...
                <p class="af-review-row__text">${review.content || review.body || ''}</p>
                <div class="af-report__actions">
                  <button class="btn btn--primary btn--sm approve-review-btn" data-id="${review.id || r.id}">✓ Aprobar</button>
                  <button class="btn btn--outline btn--sm reject-review-btn" data-id="${review.id || r.id}">✗ Rechazar</button>
                </div>
              </div>
            `;
  ...
  const styles = `
    ...
    .af-review-row__header { display: flex; align-items: center; gap: 10px; margin-bottom: 8px; }
    ...
    .af-review-row__text { font-size: 14px; color: #4b5563; margin: 0 0 12px; }
...
function initAdminForumEvents(reports, threads, reviewReports) {
  ...
  // Report filter tabs
  document.querySelectorAll('.af-ftab').forEach(tab => {
    tab.addEventListener('click', () => {
      document.querySelectorAll('.af-ftab').forEach(t => t.classList.remove('af-ftab--active'));
